...
class Memes {
		...
		if (this.$imageInput.files && this.$imageInput.files[0]) {
			...
			reader.onload = () => {
				...
				image.onload = () => {
					...
					context.strokeText(topText, this.$canvas.width/2, this.$canvas.height*(10/100));
					context.fillText(topText, this.$canvas.width/2, this.$canvas.height*(10/100));
					...
					this.reziseCanvas(this.$canvas.height, this.$canvas.width);
				};
	...
	downloadMeme() {
		...
		att.value = imageSource.replace(/^data:image\/[^;]/, 'data:application/octect-stream');
		this.$downloadButton.setAttributeNode(att);
	}

	reziseCanvas(canvasHeight, canvasWidth) {
